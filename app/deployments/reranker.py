...
from vllm.entrypoints.openai.serving_rerank import OpenAIServingRerank
...
class VLLMRerankerDeployment:
    ...
    async def rerank(self, request: RerankRequest, raw_request: Request):
        if not self._openai:
            ...
            self._openai = OpenAIServingRerank(
                engine_client=self.engine,
                model_config=cfg,
                models=models,
                request_logger=None,
        ...
        resp = await self._openai.rerank(request, raw_request)
